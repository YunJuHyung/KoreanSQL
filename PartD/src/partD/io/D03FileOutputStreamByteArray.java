...
public class D03FileOutputStreamByteArray {
	public static void main(String[] args) {
		...
		byte[] keyin = new byte[100];		//프로그램에서 지정하여 배열을 버퍼처럼 사용하기
											//바이트단위 입출력은 입출력 데이터의 양이 많을 때 실행 속도가 늦습니다.
											//프로그램에서 처리하는 단위를 100바이트로 합니다.
		
		
		
		int rBytes = 0;
		try (
			OutputStream fo = new FileOutputStream("d:/temp/do2-3.txt");	
			){
				while((r = System.in.read(keyin)) != -1) {						//r은 실제 입력된 바이트 수로 리턴합니다.
				fo.write(keyin,0,r);										//keyin 배열에서 출력할 위치 인덱스 0부터 r 바이트 만큼.
				rBytes += r;			//r 실제 입력한(읽기) 바이트수
