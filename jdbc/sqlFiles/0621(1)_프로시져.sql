...
BEGIN 	--프로시저 시작(실행문)
	...
	SELECT name,age
	INTO vname, vage	--프로시저 구문 : 검색결과를 프로시저 일반변수에 vname,vage 에저장
	FROM "TBL_CUSTOM" tc
	WHERE CUSTOM_ID = 'hongGD';		-- 1개 행만 결과 조회되는 조건
									-- 여러개 행 조회될때는 다른 cursor 필요.
	
-- DBMS_OUTPUT 은 콘솔에 출력하는 오라클의 패키지의 하나이며 PUT_LINE 함수
...
search_custom(c_id IN tbl_custom.CUSTOM_ID %TYPE)		-- 매개변수 IN 
IS 
