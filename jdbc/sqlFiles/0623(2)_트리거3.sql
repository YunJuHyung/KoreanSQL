-- // 1. 예시 : tbl_buy2 는 구매 목록 테이블 , tbl_money 는 결제 테이블 , tbl_sale 는 고객매출로 하여 
create table tbl_buy2           -- tbl_buy 구조만 복사. 시퀀스 생성은 안하고 임의로 값을 주고 테스트하겠습니다.
as
select * from tbl_buy where buy_seq=0;
...
VALUES (2000, 'mina012','CJBAb12g',3,sysdate);

-- //3. tbl_buy2 에 적용할 트리거
CREATE  OR  REPLACE  TRIGGER  set_money_trig
AFTER  INSERT  OR DELETE on tbl_buy2
FOR  EACH  ROW 
DECLARE 
    vprice number(7);
     vtotal number(7):=0;
    vcnt number(7):=0;
BEGIN 
	--DELETE FROM tbl_buy2 WHERE buy_seq =;
...
-----------------------------------------------------------------
DELETE FROM tbl_buy2 WHERE buy_seq = 1002;
DELETE FROM tbl_money WHERE money_seq = 5029;